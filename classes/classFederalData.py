...
"""Federalal Level: This file allows to draw some graphic conclusions about our data to give a visual overview of the Swiss rental market index (SRMI)
to our customers."""

# Get average price swiss wide
...
class federalData:
    ...
    def averageFederal(self, varx, varx2):
        list = []
        ...
        try:
            with DBOperations.connection.cursor() as cursor:
                sql = "SELECT {} FROM listingDetails WHERE category IN ('Apartment','Furnished apartment','Attic apartment','Maisonette','Loft','Penthouse','Terraced Apartment','Terraced condo')".format(varx)
                cursor.execute(sql)
                ...
                sql = "SELECT {} FROM listingDetails WHERE category IN ('Apartment','Furnished apartment','Attic apartment','Maisonette','Loft','Penthouse','Terraced Apartment','Terraced condo')".format(varx2)
                cursor.execute(sql)
                list = cursor.fetchall()
            ...
            cursor.close()
            print('Report for statistics regarding the SRMI in Switzerland')
            print('')

            print('Average price in Switzerland: '+str(priceavg))
            print('Average size of properties in square meters: ' + str(averageSize))
            print('Average square meter price in Switzerland: ' + str(averageMeterPrice))

    # Get the price of all the properties in the postal code area to make a distribution graph with it.
    def distribFederal(self, varx):
        list = []
        if self.dbOperations is None:
            self.dbOperations = DBOperations.getDB()
        self.dbOperations.getConnection()
        try:
            ...
            plt.title("Distribution of " + varx+ " in Switzerland")
            plt.xlabel(varx)
    ...
    def priceMeterFederal(self, varx, varx2, varx3 = None):
        dataAll = []
        scatterData = []
        ...
        try:
            with DBOperations.connection.cursor() as cursor:
                ...
                sql = "SELECT {},{},{} FROM listingDetails WHERE category IN ('Apartment','Furnished apartment','Attic apartment','Maisonette','Loft','Penthouse','Terraced Apartment','Terraced condo')".format(varx, varx2, varx3)
                cursor.execute(sql)
                data = cursor.fetchall()
           ...
           cursor.close()
           print('There are '+str(len(scatterData.varx))+' observations in Switzerland')

           #Plot the prepared data
           cmap = mpl.cm.get_cmap('RdYlBu')
           plot = plt.scatter(scatterData.varx, scatterData.varx2, s=15, c=scatterData.varx3, cmap=cmap)
           plt.colorbar(plot)
           plt.suptitle(varx + " in relation to " + varx2+" in Switzerland")
    ...
    def weatherFederal(self):
        ...
        try:
            ...
            plt.title('Yearly temperature per month in Switzerland')
            plt.xlabel('Month')
    ...
    def mapAllListingsFederal(self, shapefile=gpd.GeoDataFrame.from_file('datasets/PLZO_SHP_LV03/PLZO_OS.shp')):
        locations = []
        if self.dbOperations is None:
            self.dbOperations = DBOperations.getDB()
        self.dbOperations.getConnection()
        try:
            with DBOperations.connection.cursor() as cursor:
                ...
                cmap = mpl.cm.get_cmap('RdYlBu')
                base = shp.plot(figsize=(40, 20), linewidth=1, edgecolor='#999999', color='#ffffff')
            ...
            print("create mapAllListingsFederal SUCCESS")
            cursor.close()
            plt.show()
